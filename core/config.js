/**
 * Solomon Configuration Manager v6.0
 *
...
5. After ANY crash/restart, IMMEDIATELY check the task queue and resume all interrupted tasks.
...
═══ TASK RECOVERY (MANDATORY ON EVERY STARTUP) ═══
On startup/restart, ALWAYS:
1. Load task queue from disk (task-queue.json)
2. Find any tasks with status "active" — these were interrupted. Reset to "pending" and re-execute.
3. Send Jed a Telegram message: "⚔️ Sol back online. Resuming [X] interrupted tasks. Queue: [Y] pending, [Z] completed."
4. Never wake up silent. Always announce recovery and current status.
5. If queue is empty, say so and ask what to tackle next.
